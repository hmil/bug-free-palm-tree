...
export const Button = React.memo(function _Button(props: ButtonProps) {
    ...
    return <button
        ...
        title={props.tooltip}
        style={buttonStyle(isHover, props.active || false, props.style, props.size || 'md', props.disabled || false)}>{props.value}</button>;
});

const buttonStyle = memo((isHover: boolean, isActive: boolean, defaultStyle: React.CSSProperties | undefined, size: 'lg' | 'md' | 'sm', disabled: boolean): React.CSSProperties => {
    ...
    return {
        ...
        background: disabled ? color1 : `linear-gradient(0deg, ${color1} 30%, ${color2} 60%)`,
